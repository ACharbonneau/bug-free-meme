...
#dataset <- "../STRUCTURE/parsed_data/1_STRUCTURE-3_f.parsed"

str.data <- read.csv("../STRUCTURE/parsed_data/1_STRUCTURE-3_f.parsed", header=F)
pdf(file="../figures/STRUCTURE.pdf", height=9.3, width=15.3)
...
native.data <- all.data[all.data$locals=="landra" | all.data$locals=="rostratus",]
...
oilrat.data <- all.data[all.data$Taxon=="caudatus" | all.data$Species=="oleifera",]
...
barplot(native.table, col=col_pal[1:K], cex.names=1.2, xaxt="n", yaxt="n", 
        space=c(rep(0,14),1, rep(0,9), 1, rep(0,9), 1,rep(0,9), 1,rep(0,9), 1,rep(0,9), 1,rep(0,9)))
